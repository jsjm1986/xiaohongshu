...
export function evaluatePlanToCopyAlignment(
  ...
  const briefCheck = !imageBriefEnabled
    ...
    : !content.N.imageBrief.trim()
      ...
      : anyConnection(imageAnchors, content.N.imageBrief)
        ...
        : check("image_plan_to_brief", "warn", "未找到图片说明与计划锚点的可解释连接；中文改写可能造成词面差异，需人工复核而不直接判失败。", imageAnchors);
  ...
  const coverCheck = coverAnchors.length === 0
    ...
    : anyConnection(coverAnchors, titleAndBody)
      ...
      : check("cover_promise_to_copy", "warn", "未找到封面承诺与标题/正文的可解释连接；仅提示复核，不把词面不一致当作硬失败。", coverAnchors);
  ...
  const contentCheck = contentAnchors.length === 0
    ...
    : contentAnchors.some((anchor) => semanticallyConnected(anchor, titleAndBody))
      ...
      : check("planned_anchor_to_copy", "warn", "未找到计划内容锚点在标题/正文中的可解释承接；这可能是同义改写，需人工复核。", contentAnchors);
  ...
  const connectedBoundaries = boundaries.filter((boundary) => semanticallyConnected(boundary, visibleCopy));
  const boundaryCheck = boundaries.length === 0
    ...
    : contradictions.length > 0
      ...
      : connectedBoundaries.length > 0
        ? check("boundary_continuity", "pass", "可见文案承接了至少一项计划边界，且未发现明确反向要求。", boundaries)
        : check("boundary_continuity", "warn", "未在可见文案中找到边界的可解释承接；边界也可能作为隐式守则生效，因此只提示人工复核。", boundaries);
