...
import { candidateQualityStatus, isNonOverridableContentIssueCode } from '@content-agent/agent-core';
...
export class V1Controller {
  ...
  contentPackage(@Req() rawRequest: Request, @Param('id') id: string): Record<string, unknown> {
    ...
    return {
      ...
      qualityStatus: candidateQualityStatus({ valid: validation?.valid, issues: validation?.issues ?? [] }),
      content: pkg,
      createdAt: row.created_at,
      updatedAt: row.updated_at,
    };
