...
export function IntelligentSimpleFlow({ projects, projectId, selectedPresetId, selectedPreset, submitting, onProject, onPreview }: Props) {
  ...
  return <div className="intelligence-flow v2-protocol">
    ...
    <Modal open={poolOpen} onClose={() => setPoolOpen(false)} title="内容智能资源池" description="信息缺口决定写什么，完整表达策略决定标签、图文与评论怎样协同。" size="wide">
      <div className="pool-manager"><div className="pool-tabs"><button type="button" className={poolTab === "gaps" ? "active" : ""} onClick={() => setPoolTab("gaps")}>信息缺口池 <b>{gaps.length}</b></button><button type="button" className={poolTab === "strategies" ? "active" : ""} onClick={() => setPoolTab("strategies")}>完整表达策略池 <b>{strategies.length}</b></button></div><div className="pool-toolbar"><label><Search size={15} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索资源" /></label><Button icon={<Plus size={15} />} onClick={() => { if (poolTab === "gaps") { setEditingGap({}); } else { setEditingStrategy({}); } }}>新增</Button>{poolTab === "gaps" && pendingGapCount > 0 && <Button variant="secondary" icon={<WandSparkles size={15} />} onClick={() => openEnrich()}>{enrichButtonLabel(pendingGapCount, gapTiers)}</Button>}</div>
