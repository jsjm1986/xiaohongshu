...
export function ProjectsPage() {
  const { projects, projectId, setProjectId, addProject, updateProject, removeProject } = useProjects();
          ...
          <article className={`project-card ${project.id === projectId ? 'project-card--active' : ''}`} key={project.id}>
            <header><div className={`project-card__mark project-card__mark--${index % 3}`}><Boxes size={21} /></div>
              <div className="card-menu">
                <button className="icon-button" onClick={() => setMenuFor(menuFor === project.id ? null : project.id)}><MoreHorizontal size={19} /></button>
                {menuFor === project.id && (
                  <div className="card-menu__pop" onMouseLeave={() => setMenuFor(null)}>
                    <button onClick={() => { setEditing({ id: project.id, name: project.name, domain: project.domain || '', description: project.description || '' }); setMenuFor(null); }}><Pencil size={14} /> 编辑</button>
                    <button className="danger" onClick={() => { setDeleting({ id: project.id, name: project.name }); setMenuFor(null); }}><Trash2 size={14} /> 删除</button>
                  </div>
                )}
              </div>
            </header>
