...
const normalizeGap = (raw: JsonRecord): InformationGap => {
  ...
  return {
    ...
    sourceStatus: ["supplied_fact", "user_supplied", "inference", "hypothesis", "unknown"].includes(String(data.sourceStatus))
      ...
      : undefined,
    knowledgeAction: ["organize_existing", "ask_user", "none"].includes(String(data.knowledgeAction))
      ? (data.knowledgeAction as InformationGap["knowledgeAction"])
      : "none",
    knowledgeReason: typeof data.knowledgeReason === "string" ? data.knowledgeReason : undefined,
    createdAt: typeof raw.createdAt === "string" ? raw.createdAt : undefined,
    updatedAt: typeof raw.updatedAt === "string" ? raw.updatedAt : undefined,
    status,
...
export const api = {
  ...
  authorFacts: {
    organize: (projectId: string, narrative: string) =>
      request<AuthorFactOrganizationResult>(
        `/api/projects/${encodeURIComponent(projectId)}/author-facts/organize`,
  ...
  generations: {
    ...
    /** 撤销软删。 */
    restore: (id: string) =>
      request<{ id: string; topic: string; alreadyActive: boolean }>(
        `/api/generations/${encodeURIComponent(id)}/restore`,
        { method: "POST" },
      ),
    exportUrl: (
      id: string,
