...
test('保存后的说明不承诺缺口会消失', () => {
  /*
   * 三次真实模型冒烟都出现同一件事:补充保存、重新分析之后,原先空白的缺口
   * 仍是 unknown。核对过,这是对的——知识图谱确实读到了新文件,并正确标成
   * factEligible:false,因为草稿如实写的是「资料未提及」而不是编造答案。
   *
   * 但旧文案说「建议重新分析以让补充内容生效」,用户照做后发现缺口还在,
   * 会判定功能坏了。所以这里禁的是「生效」这类承诺式说法,要求写明
   * 「补充的是待确认内容,缺口要填真事实才会关闭」。
   */
  const source = read('../src/components/knowledge/KnowledgeEnrichmentModal.tsx');
  assert.doesNotMatch(
    source,
    /建议重新分析以让补充内容生效/,
